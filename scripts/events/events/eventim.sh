...
EVENTS_FILE="${SCRIPT_DIR}/eventim_events.json"
...
while IFS= read -r event_json; do
  if ! http_code="$(
    ...
    echo "eventim: request failed before receiving an HTTP response" >&2
    exit 1
  fi
  ...
  case "${http_code}" in
    ...
    409)
      ...
      ;;
    *)
      printf 'eventim: request %d failed with HTTP %s\n' "${processed_count}" "${http_code}" >&2
      cat "${response_file}" >&2
      exit 1
      ;;
  esac
  ...
  if (( processed_count % 100 == 0 )); then
    printf 'eventim: processed %d events (%d created, %d duplicates)\n' "${processed_count}" "${created_count}" "${duplicate_count}"
  fi
...
if (( duplicate_count > 0 )); then
  printf 'eventim: duplicate events detected in %d documents\n' "${duplicate_count}" >&2
  exit 1
fi
