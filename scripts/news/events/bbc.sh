...
EVENTS_FILE="${SCRIPT_DIR}/bbc_events.json"
...
while IFS= read -r event_json; do
  if ! http_code="$(
    ...
    echo "bbc: request failed before receiving an HTTP response" >&2
    exit 1
  fi
  ...
  case "${http_code}" in
    ...
    409)
      ...
      ;;
    *)
      printf 'bbc: request %d failed with HTTP %s\n' "${processed_count}" "${http_code}" >&2
      cat "${response_file}" >&2
      exit 1
      ;;
  esac
  ...
  if (( processed_count % 100 == 0 )); then
    printf 'bbc: processed %d events (%d created, %d duplicates)\n' "${processed_count}" "${created_count}" "${duplicate_count}"
  fi
...
if (( duplicate_count > 0 )); then
  printf 'bbc: duplicate events detected in %d documents\n' "${duplicate_count}" >&2
  exit 1
fi
